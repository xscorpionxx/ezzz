...
class Stateaddservices extends State<Chat>{
  ...
  var type;
  var userid;
  Stateaddservices({this.id ,this.type});
...
// Laravel echo will subscribe the channel with full namespace.
  ...
  getmessages() async{
   messages = await chatlogic.getmessages(id);
   setState(() {});
  }
  @override
  void initState() {
    ...
    return Scaffold(body: Container(width:  MediaQuery.of(context).size.width,height: MediaQuery.of(context).size.height,
      child:ListView(children: [
            Container(width: MediaQuery.of(context).size.width,height: MediaQuery.of(context).size.height*0.15,
              child: Row(children: [
                Expanded(child: Container(margin: EdgeInsets.only(left: MediaQuery.of(context).size.width*0.05),child: IconButton(icon: Icon(Icons.arrow_back,size: MediaQuery.of(context).textScaleFactor*18,color: Colors.black,),onPressed: (){navigateback();},))),
