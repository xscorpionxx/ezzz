...
class Editcoupons extends StatefulWidget{
  List services =[];
  var data ;
  Editcoupons({this.services ,this.data});
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return StateEditcoupons(services: services ,data:data);
  }
}
class StateEditcoupons extends State<Editcoupons>{
  List services =[];
  var data ;
  StateEditcoupons({this.services ,this.data});
  var val = "ريال";
  List vals = ["ريال"];
  ...
  List servicesnow =[];
  getdata(){
    cuponame.text = data["CouponCode"].toString();
    price.text = data["CouponValue"].toString();
    servicesnow = data["CouponService"];
    for(int f =0 ; f<services.length ;f++){
      checks.add(false);
    }
    for(int f =0 ; f<services.length ; f++){
      for(int g =0 ; g<servicesnow.length ; g++){
        if(services[f]["ID"].toString() == servicesnow[g]["ID"].toString()){
          checks[f] = true;
        }
      }
    }
      ...
      child: Form(key: key,
        child: ListView(children: [
          Container(width: MediaQuery.of(context).size.width,height: MediaQuery.of(context).size.height*0.12,child :Stack(children: [
            Container(width: MediaQuery.of(context).size.width,height: MediaQuery.of(context).size.height*0.15,
              decoration: BoxDecoration(color:Color.fromRGBO(42, 171, 227, 1),borderRadius: BorderRadius.only(bottomRight: Radius.circular(70),bottomLeft: Radius.circular(70))),child: Row(children: [
                ...
                Expanded(flex: 5,child: Container(alignment: Alignment.center,child: Text("تعديل كوبون خصم",style: TextStyle(color: Colors.white,fontSize: MediaQuery.of(context).size.width*0.045),))),
          ...
          InkWell(onTap: (){
            ...
            addcuponlogic.editcupon(key,cuponame.text , price.text ,service,data["CouponID"] );
          },
            child: Container(height: MediaQuery.of(context).size.height*0.07,alignment: Alignment.center,decoration: BoxDecoration(color: Color.fromRGBO(69, 190, 0, 1),),margin: EdgeInsets.all(MediaQuery.of(context).size.width*0.1),
              child:InkWell(child: Container(child: Text("تعديل ", style: TextStyle(color: Colors.white,fontSize: MediaQuery.of(context).size.height*0.025)))) ,),
          )
        ],),
      ),
    ),);
